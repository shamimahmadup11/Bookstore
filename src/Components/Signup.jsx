...
const Signup = () => {
  ...
  const handleSignup = async (signupData) => {
    ...
    if (!signupData.fullname) {
      errors.push({ fullname: "name", massage: "Name is required" })
    }
    if (!signupData.email) {
        ...
        <div className="modal-box bg-gray-100 flex flex-col gap-4 border ">
          ...
          <div>
            ...
            <p className="text-red-600 ">
              {errors?.find((err) => err.fullname === "name")?.massage}
            </p>
          </div>
          <div>
          ...
          <div className="flex justify-around">
            ...
            <Link className=" bg-pink-600 rounded h-12 w-24 px-5 py-3 mt-4 cursor-pointer text-white" onClick={() => handleSignup(signupData)}>
              Signup
            </Link>
